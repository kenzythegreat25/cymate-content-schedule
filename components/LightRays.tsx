...
export default function LightRays({
  ...
  // IntersectionObserver — only run WebGL when the section is on screen
  useEffect(() => {
  ...
  gl_FragColor = fragColor;
      ...
      const loop = (t: number) => {
        ...
        if (followMouse && mouseInfluence > 0) {
          const k = 0.92;
          smoothMouseRef.current.x = smoothMouseRef.current.x * k + mouseRef.current.x * (1 - k);
          smoothMouseRef.current.y = smoothMouseRef.current.y * k + mouseRef.current.y * (1 - k);
